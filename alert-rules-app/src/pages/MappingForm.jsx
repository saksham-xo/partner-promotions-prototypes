  ...
  return (
    <div>
      <div className="bg-surface rounded-lg shadow-[0_0_1px_1px_var(--color-border)] p-4 px-6 flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-semibold text-text">{mappingId ? 'Edit Mapping' : 'Add Mapping'}</h1>
          <p className="text-sm text-text-secondary mt-0.5">{manageMatchKeyTarget?.name || 'Lookup Attribute'} → Product Code mapping</p>
        </div>
      </div>
      ...
      <div className="bg-surface rounded-lg shadow-[0_0_1px_1px_var(--color-border)]">
        ...
        <div className="p-6 flex flex-col gap-4">
          <div>
            <label className="text-xs font-semibold text-text-secondary block mb-1.5">{manageMatchKeyTarget?.name || 'Lookup Attribute'} Value</label>
            <input
              type="text"
          ...
          <div>
            <label className="text-xs font-semibold text-text-secondary block mb-1.5">Product Code</label>
            <input
              type="text"
              value={draft.skuCode}
              onChange={e => setDraft(prev => ({ ...prev, skuCode: e.target.value }))}
